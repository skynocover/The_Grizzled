...
func init() { //宣告狀態機
	...
	Round.Status = fsm.NewFSM(
		...
		fsm.Events{
			...
			{Name: "LuckyClover", Src: []string{"Mission"}, Dst: "LuckyClover"},    //抽完牌遊戲開始
			{Name: "LuckyCloverEnd", Src: []string{"LuckyClover"}, Dst: "Mission"}, //抽完牌遊戲開始
...
func (this *tRound) SupportEnd(playnow *player, choose string) {
	...
	case "Threat":
		...
		for i := range playnow.threat {
			if i > cancel || i > len(playnow.threat) {
				break
			}
...
func (this *tRound) SpeechCard(player *player, choose int) bool {
	...
	if this.tmp == this.playerlist.Len()-empty {
...
func (this *tRound) PlayCard(playernow *player, num int) {
	...
	if len(playernow.Handcard) == 0 {
		playernow.status = "empty" //沒手牌
	}
}
